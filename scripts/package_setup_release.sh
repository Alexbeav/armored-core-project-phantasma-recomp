...
#
# Writes: dist/armoredcoreprojectphantasma-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix armoredcoreprojectphantasma \
  --exe-name Armored_Core_Project_Phantasma \
  --omit-openbios \
  --display-name "Armored Core: Project Phantasma" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Armored Core: Project Phantasma (USA) disc" \
  --runtime-dir mods \
